  ...
  useEffect(() => {
    const loadAndFilterData = async () => {
      ...
      try {
        ...
        if (!imagesResponse.ok) throw new Error(`Failed to load images: ${imagesResponse.statusText}`);
        ...
        if (categoryIdParam) {
          ...
          if (!isNaN(categoryId)) {
            ...
            const categoryIdsToShow = getAllDescendantIds(categoryId, categoriesData);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {displayedProducts.length > 0 ? (
          displayedProducts.map((product) => {
            ...
            const productImage = allProductImages.find( // Search the state variable
              (img: ProductImage) =>
