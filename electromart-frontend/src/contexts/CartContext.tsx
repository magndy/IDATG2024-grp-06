...
// Import the Product type - adjust path if your types are elsewhere
import { Product } from '../data/mockData';

// 1. Define the shape of an item in the cart
export interface CartItem extends Product { // Extends Product to include all its details
  quantity: number;
}

// 2. Define the shape of the context value (state and functions)
...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  ...
  const removeFromCart = (productId: number | string) => {
    setCartItems((prevItems) =>
     ...
     console.log(`Removed item ${productId} from cart. Current items:`, cartItems); // Log for debugging
  };
  ...
  const updateQuantity = (productId: number | string, quantity: number) => {
    setCartItems((prevItems) => {
     ...
     console.log(`Updated quantity for item ${productId} to ${quantity}. Current items:`, cartItems); // Log for debugging
  };
